...
def summonjin(username_arr : list, password_arr: list, role_arr: list) -> tuple : 
    count = 0
    for i in range(102):
        if username_arr[i] != "%" :
            count += 1
    ...
    if count == 102 : 
        ...
        if "1" <= n <= "2" :
            ...
            while valid : # akan looping terus menerus jika valid = True yang artinya username terdapat di username_arr
                print("Username \"" + unamejin + "\" sudah diambil!")
                unamejin = str(input("\nMasukkan username jin: "))
                for i in range(102) :
            ...
            if valid == False :
                ...
                while not (5 <= x <= 25): # akan dilakukan looping terus menerus jika password yang dimasukkan tidak diantara 5 sampai 25 karakter
                    ...
                    x = len(passjin)
            for i in range (102):
                if username_arr[i]=="%": # jika semua sudah valid, maka dilakukan ini agar username, password dan role akan disimpan ke dalam array 
